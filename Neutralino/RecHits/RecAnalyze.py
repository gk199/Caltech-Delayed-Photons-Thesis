...
unmatchedjetetapuppitiming = ROOT.TH1D("unmatchedjetetapuppitiming","",50,-5.,5.)
unmatchedjetetapuppinotiming = ROOT.TH1D("unmatchedjetetapuppinotiming","",50,-5.,5.)
...
for event in events2:
    ...
    for cand in pfcands:
        ...
        if abs(cand.charge())>0.:
            ...
            if abs(cand.dz(0))<0.1 and (not usetime or abs(cand.dtime(0))<0.09):
            #if abs(cand.dz(0))<0.1:
                candetanotimingsimple.Fill(cand.eta())
