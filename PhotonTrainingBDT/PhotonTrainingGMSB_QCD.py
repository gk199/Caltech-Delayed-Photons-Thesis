...
#signal
signalFileName = '/Users/cmorgoth/Work/GillianThesis/GMSB_L250TeV_Ctau200cm_13TeV-pythia8_skimmedpho1Pt_EB.root'
signalFile = root.TFile(signalFileName)
signalTree = signalFile.Get('DelayedPhoton')
signalNevents = signalFile.Get('NEvents')

#bkg
bkgFileName = '/Users/cmorgoth/Work/GillianThesis/DelayedPhoton_QCD_HTall_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_skimmed_skimmedpho1Pt_EB.root'
bkgFile = root.TFile(bkgFileName)
bkgTree = bkgFile.Get('DelayedPhoton')
bkgNevents = bkgFile.Get('NEvents')

test_name = 'Photons_GMSB_QCD_ECAL'
...
plt.plot(fpr, tpr, color='darkorange',
         lw=lw, label=r'GMSB vs QCD, with ECAL timing')
plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.xlabel('Background (false positives)')
plt.ylabel('Signal (true positives)')
plt.title(r'ROC - GMSB vs. QCD, $\Lambda$ = 250 TeV, $c\tau$ = 200 cm')
