...
GJets_FileName = '/Users/cmorgoth/Work/GillianThesis/DelayedPhoton_GJets_HTall_TuneCUETP8M1_13TeV-madgraphMLM-pythia8_skimmed_skimmedpho1Pt_EB.root'
bkgFile = root.TFile(GJets_FileName)
bkgTree = bkgFile.Get('DelayedPhoton')
bkgNevents = bkgFile.Get('NEvents')
test_name = 'Photons_modelGMSB_QCD_X'
...
# set the cuts to use
cuts1 = np.arange(0, 0.1, 0.01)
cuts2 = np.arange(0.1, 0.9, 0.001)
cuts3 = np.arange(0.9, 0.93, 0.00001)
cuts4 = np.arange(0.93, 0.98, 0.0000005)
cuts5 = np.arange(0.98, 1, 0.0000005)
cuts = np.concatenate([cuts1, cuts2, cuts3, cuts4, cuts5])
...
plt.figure()
lw = 2
plt.semilogx(eff_bkg, eff_sig, color='darkorange',
...
plt.title(r'ROC - GMSB vs. QCD (model)')
...
plt.figure()
lw = 2
plt.semilogx(eff_GJets, eff_sig, color='darkorange',
         lw=lw, label=r'GMSB vs $\gamma$+jets')
plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')
plt.xlim([0.0, 1.0])
plt.ylim([0.0, 1.05])
plt.grid()
plt.xlabel('Background (false positives)')
plt.ylabel('Signal (true positives)')
plt.title(r'ROC - GMSB vs. $\gamma$ + Jets (with GMSB vs. QCD model)')
...
plt.figure()
lw = 2
plt.semilogx(eff_bkg, eff_GJets, color='darkorange',
...
plt.xlabel('Background (false positives)')
plt.ylabel('Signal (true positives)')
plt.title(r'ROC -  $\gamma$ + Jets vs. QCD (with GMSB vs. QCD model)')
plt.legend(loc="lower right")
#plt.show()
